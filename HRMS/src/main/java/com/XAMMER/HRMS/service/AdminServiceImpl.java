...
public class AdminServiceImpl implements AdminService {
    ...
    public User getTodayAttendance(String username) {
        User user = userRepository.findByUsername(username).orElseThrow(() -> new IllegalArgumentException("User not found."));
        LocalDate today = LocalDate.now(ZoneId.of("Asia/Kolkata"));
        return attendanceRepository.findByUserIdAndAttendanceDate(user.getId(), today).orElse(null);
    }
    ...
    public List<String> getRecentActivity() {
        List<String> latestUsersActivity = userRepository.findAll().stream()
                .limit(5)
                .map(user -> "New user registered: " + user.getFirstName() + " " + user.getLastName())
        ...
        List<String> latestCheckInsActivity = attendanceRepository.findAll().stream()
                ...
                .limit(5)
                .map(attendance -> attendance.getUser().getFirstName() + " checked in at " + attendance.getCheckInTime().toLocalTime())
                .collect(Collectors.toList());
